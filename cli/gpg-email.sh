...
# echo "$message" | gpg2 --no-emit-version -eas -r hisamafahri@gmail.com -r $user | mail -s "$subject" $email

# Generate a timestamp for the filename
timestamp=$(date +%Y%m%d%H%M%S)
filename="output_${timestamp}.gpg"

echo "$message" | gpg2 --no-emit-version -eas -r hisamafahri@gmail.com -r $user > "$filename"
